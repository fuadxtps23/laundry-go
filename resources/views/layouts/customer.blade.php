...
<div class="min-h-screen lg:flex">
    <aside id="customer-sidebar" class="fixed inset-y-0 left-0 z-50 w-72 -translate-x-full bg-white shadow-xl transition-transform lg:static lg:translate-x-0 lg:shadow-none">
        <div class="flex h-full flex-col border-r border-slate-200">
            <div class="flex items-center justify-between border-b border-slate-100 px-6 py-6">
                <a href="{{ route('customer.dashboard') }}" class="flex items-center gap-3"><span class="flex h-10 w-10 items-center justify-center rounded-xl bg-brand-500 font-black text-white">LG</span><span><span class="block text-lg font-extrabold text-brand-900">Laundry<span class="text-brand-500">Go</span></span><span class="block text-[10px] uppercase tracking-[0.2em] text-slate-400">Customer area</span></span></a>
            ...
            <nav class="flex-1 space-y-1 overflow-y-auto px-4 py-6">
                ...
                @endphp
                @foreach ($customerNav as [$label, $routeName, $icon])
                    @php $active = request()->routeIs($routeName) || request()->routeIs($routeName.'.*'); @endphp
                    <a href="{{ route($routeName) }}" class="flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold transition {{ $active ? 'bg-brand-100 text-brand-800' : 'text-slate-500 hover:bg-brand-50 hover:text-brand-700' }}"><span class="w-5 text-center text-base">{{ $icon }}</span>{{ $label }}</a>
                @endforeach
            </nav>
            <div class="border-t border-slate-100 p-4"><form method="POST" action="{{ route('logout') }}">@csrf<button class="flex w-full items-center gap-3 rounded-xl px-3 py-2 text-sm font-semibold text-slate-500 hover:bg-red-50 hover:text-red-600">↪ <span>Keluar</span></button></form></div>
        </div>
    </aside>
    ...
    <div class="min-w-0 flex-1">
        <header class="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-slate-200 bg-white/90 px-4 backdrop-blur sm:px-6 lg:px-8">
            <div class="flex items-center gap-3"><button id="open-customer-sidebar" class="rounded-xl border border-slate-200 p-2 text-slate-600 lg:hidden">☰</button><div><p class="text-sm font-semibold text-slate-900">@yield('header', 'Dashboard')</p><p class="hidden text-xs text-slate-400 sm:block">{{ now()->translatedFormat('l, d F Y') }}</p></div></div>
            <div class="flex items-center gap-3"><a href="{{ route('customer.orders.create') }}" class="btn-primary hidden !px-3 !py-2 sm:inline-flex">＋ Pesan laundry</a><span class="flex h-9 w-9 items-center justify-center rounded-full bg-brand-100 text-sm font-bold text-brand-700">{{ strtoupper(substr($user->nama_lengkap, 0, 1)) }}</span></div>
        </header>
        <main class="p-4 sm:p-6 lg:p-8"><x-flash />@yield('content')</main>
    </div>
</div>
<script>
