...
@section('eyebrow', 'Karyawan portal')
@section('headline', 'Kelola operasional laundry dengan rapi.')
@section('footer', 'Khusus karyawan Laundry Go. <a class="font-semibold text-brand-600" href="'.route('home').'">Kembali ke situs</a>')

@section('form')
    <div class="mb-7"><p class="text-sm font-semibold text-brand-600">Akses karyawan</p><h2 class="mt-1 text-2xl font-bold tracking-tight text-slate-900">Masuk sebagai karyawan</h2><p class="mt-2 text-sm text-slate-500">Pantau pesanan dan proses laundry harian.</p></div>
    <form method="POST" action="{{ route('karyawan.login.store') }}" class="space-y-5">
        @csrf
        ...
        <button class="btn-primary w-full py-3">Masuk ke panel <span>→</span></button>
    </form>
@endsection
